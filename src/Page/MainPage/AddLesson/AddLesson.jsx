...
import { maxTitleLength, maxDescriptionLength } from '@utils/constants';
...
function AddLesson() {
    ...
    const { isAuthorized, isServerDown } = useAuthCheck();
    ...
    const handleAutoTranslateToggle = () => {
        if (isServerDown && !isLocal) {
            setMessage('Sorry, the server is currently unavailable.');
            setShowMessage(true);
            return;
        }

        if (isLocal || !isAuthorized) {
            setMessage('To use the translator, you need to log in.');
            setShowMessage(true);
            return;
        }
        ...
        <div className={style.addLesson}>
            ...
            <div className={isImport ? style.isHide : ''}>
                ...
                <div className={style.form}>
                    <input onChange={(e) => setLessonTitle(e.target.value)} className={style.input} type="text" placeholder="Lesson Title" maxLength={maxTitleLength} />
                    <textarea onChange={(e) => setLessonDescription(e.target.value)} className={style.textarea} ref={textareaRef} placeholder="Description" maxLength={maxDescriptionLength}></textarea>
                </div>
                    ...
                    <SortableContext items={normalWords.map(w => w.id)} strategy={verticalListSortingStrategy}>
                        <ul className={style.words}>
                            {Words.map((word, index) => (
                                <li key={word.id}>
                                    <Card ref={el => wordRefs.current[index] = el} id={word.id} index={index} term={word.term} definition={word.definition}
